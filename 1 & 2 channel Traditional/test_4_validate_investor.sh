#!/bin/bash
# TEST 4: ValidateInvestor — Org3
# Sig: ValidateInvestor(investorID, decision)
echo "============================================"
echo "TEST 4: VALIDATE INVESTOR (ORG3 - VALIDATOR)"
echo "============================================"
...
echo "Validating 20 Investors (I100–I119)"
echo "============================================"
for ((i=100;i<120;i++)); do
    peer chaincode invoke \
      ...
      -c "{\"Args\":[\"ValidateInvestor\",\"I$i\",\"APPROVED\"]}" 2>/dev/null
...
echo "TEST 4 RESULTS"
echo "============================================"
echo "📊 Investors Validated: $success/$total"
echo "📊 Failed: $failed/$total"
echo "📊 Time Taken: ${elapsed}s"
echo "📊 TPS: $tps"
echo "📊 Avg Latency: ${latency}ms"
echo "📊 Success Rate: $(echo "scale=2; ($success/$total)*100" | bc)%"
if [ $failed -eq 0 ]; then echo "✓ ALL INVESTORS VALIDATED SUCCESSFULLY! 🎉"; echo "ℹ Run ./test_5_create_project.sh next"
else echo "⚠ Some failed."; fi
RESULT_FILE="test4_validate_investor_results_$(date +%Y%m%d_%H%M%S).txt"
{ echo "TEST 4: VALIDATE INVESTOR RESULTS"; echo "Timestamp: $(date)"; echo "Investors Validated: $success/$total"; echo "Failed: $failed/$total"; echo "Time Taken: ${elapsed}s"; echo "TPS: $tps"; echo "Avg Latency: ${latency}ms"; echo "Success Rate: $(echo "scale=2; ($success/$total)*100" | bc)%"; } > "$RESULT_FILE"
